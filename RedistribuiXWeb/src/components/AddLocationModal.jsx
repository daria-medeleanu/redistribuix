...
export default function AddLocationModal({ isOpen, onClose, onSuccess }) {
    ...
    const [standManagerErrors, setStandManagerErrors] = useState({
        name: '',
        email: '',
        password: '',
        confirmPassword: ''
    ...
    const handleStandManagerChange = (e) => {
        ...
        } else if (name === 'smName' || name === 'smEmail' || name === 'smPassword' || name === 'smConfirmPassword') {
    ...
    const validateStandManager = () => {
        ...
        if (!standManagerData.name.trim()) {
            errors.name = 'Stand Manager name is required'
            hasError = true
        }

        if (!validateEmail(standManagerData.email)) {
            errors.email = 'Valid email is required'
            hasError = true
        }

        if (!validatePassword(standManagerData.password)) {
            errors.password = 'Password must be at least 6 characters'
            hasError = true
        }

        if (standManagerData.password !== standManagerData.confirmPassword) {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate stand manager if needed
        if (!validateStandManager()) {
            hasError = true
        }
        ...
        try {
            ...
            console.log(locationPayload, isSpecific ? '(specific address)' : '(locality center)');
            ...
            // Create stand manager if data is provided
            if (standManagerData.addStandManager) {
                const standManagerPayload = {
                ...
                const smResponse = await fetch(`${API_BASE}/StandManager`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(standManagerPayload)
                })

                if (!smResponse.ok) {
                    const errorData = await smResponse.text()
                    throw new Error(errorData || 'Failed to create stand manager')
                }

                console.log('Stand manager created successfully with ID:', await smResponse.json())
            }
            ...
            setCountyError('')
            setLocalityError('')
            setStandManagerErrors({ name: '', email: '', password: '', confirmPassword: '' })
            onSuccess()
            onClose()
        } catch (error) {
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="border-t border-slate-200 pt-4 mt-4">
                        <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span className="text-sm font-medium text-slate-700">Add Stand Manager</span>
                        </label>
                    </div>
                    ...
                    {standManagerData.addStandManager && (
                        <div className="space-y-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Stand Manager Name</label>
                                <input
                                    type="text"
                                    name="smName"
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={handleStandManagerChange}
                                        className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-1 ${
                                            standManagerErrors.confirmPassword
                                                ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                                                : 'border-slate-300 focus:border-blue-500 focus:ring-blue-500'
                                        }`}
                                    />
